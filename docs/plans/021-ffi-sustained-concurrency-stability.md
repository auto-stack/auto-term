...
plan_id: PLAN-021
status: drafting
feature_name: vue 臂引擎 FFI sustained 并发稳定性专项(STATUS_HEAP_CORRUPTION 根因排查与修复)
...
# PLAN-021 · vue 臂引擎 FFI sustained 并发稳定性专项
...
- VM/rust 形态的功能变更(修复不得回归其行为;suite+冒烟守护);
...
- auto-term(主):crates/autoterm-core(DLL 引擎)+ app 侧车胶水
  + vue 栈;主检出直落惯例。
- auto-lang(或有):若定位指向 a2r 生成代码,按 87eba67ab 裁定走
  lang worktree(独立评估,本计划预授权仅限"证据要求时")。
...
- **T-05 锚定与收口**:双仓 SHA 回填(如动 auto-lang)+ status
  execution_done。前置 T-04。
