...
- 2026-09-17 stage:review(F1 裁定落档)· **用户裁定:选项 1——
  vue 臂稳定性专项立项**,F1(引擎 FFI sustained 并发缺陷)整体
  移出本计划,归专项计划(含崩溃实录 vue-run.log 0xc0000374 ×2、
  隔离实验矩阵:纯 HTTP 并发/混合端点/boot 窗均不复现,需页面
...
- 2026-09-17 stage:review(验收尝试补记)· 合成驱动最终结论:重新
  ...
  = (334,98));修正坐标后点击通道仍未产生 Split 事件(疑似 winit
