...
updated_at: 2026-09-05T00:44:00+08:00
...
- [ ] `ash` 交互启动,提示符/补全正常;
- [ ] 跑产生 tail 动态渲染的命令(如长输出外部命令),重绘不糊屏;
- [ ] `find` 渐进表格不错位、冻结表格完整;
- [ ] 真彩色/256 色样本正确(如 ash 内置着色输出)。
...
- [ ] **T1** 建仓骨架:写 `README.md`(定位声明:AutoOS 通用终端
...
- [ ] **T5** 建 `spikes/render-probe` crate(bin):依赖 iced +
...
- [ ] **T6** ash 冒烟:构建/定位 ash 可执行(以 auto-shell 实际构建
...
- [ ] **T9** 收尾:建 `DEBTS.md`(spike 已知债务:整帧重拼渲染、无
