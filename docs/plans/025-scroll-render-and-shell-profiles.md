...
- [x] **T-04** 滚动即时泵(视 T-01 判决取舍;若主因纯属候选 1 可记
  ...
  判决豁免记录。
  [✅ 豁免→启用 2026-09-20]一轮按判决豁免(残余仅极端连滚薄带,
  50ms 泵兜底);**二轮实机(快拖半屏空白)触发启用条款**。实现 =
  泵节拍门:计时器 16ms + `db.tick_gate`(满拍 ≥3 拍 ≈48ms 等阶旧
  50ms,或可见 Pane 滚动增量待排即满拍;peek 探针不排空)+ 预取
  N 8→24。提交:lang-025 3f3187885 + auto-term 65bd133;模型断言
  翻转(两档速率空白带 0 行 0 帧)+ pending 探针语义测试;p025 9/9。
  §10.5 DEBT 闭合。
