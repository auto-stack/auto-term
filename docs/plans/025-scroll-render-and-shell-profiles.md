...
- [x] **T-04** 滚动即时泵(视 T-01 判决取舍;若主因纯属候选 1 可记
  ...
  判决豁免记录。
  [✅ 终态 2026-09-20 · 三轮迭代]一轮豁免(50ms 兜底)→ 二轮
  (快拖半屏空白)触发 16ms 节拍门 + 预取 N 8→24 → **三轮修正:
  **split 形态**下 16ms 计时器 = 每拍 HTTP 同步往返,占死前端 UI
  线程(无响应 + 内存爬升,用户实录;canary 实锤 pac api: rust
  经 run 命令 set_var 覆写 env 锁定 VM 轨 split——merged/split
  为正交轴,两轨皆有,用户勘正)→ 计时器回恒 50ms 去门**;快滚
  覆盖由预取 N=24 独担(模型:50ms 泵两档速率 lead ≤15 行 < 24,
...
- **T-05** A 部门禁:022/023 回归 + 三轨零回归 + 实机滚动录屏与
  ...
  p025 8/8 + terminal 系 48 绿;载具重建 boot 冒烟绿。
  [二轮复测 2026-09-20]抖动基本消失;新报快拖时内容半屏空白——
...
5. **T-04 即时泵 DEBT**:[已闭合 2026-09-20]二轮实机(快拖半屏空白)
   触发启用条款——泵节拍门落地(见 T-04/spec §6),DEBT 清账。
